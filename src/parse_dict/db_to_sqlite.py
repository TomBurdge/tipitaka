...
def is_pyglossary_installed():
    try:
        ...
        answer = input("PyGlossary is not installed. Do you want to install it? (y/n) ")
        if answer.lower() == "y":
...
def remove_title_and_body_tags(text: str) -> str:
    cleaned_text = text.replace("<title>Digital Pāḷi Dictionary</title><body>", "")
...
def dpd_to_sqlite_main(tab_file: str = "dpd.txt") -> None:
    ...
    with open(tab_file, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if "</style>" in defi:
                ...
                if css not in style_tags:
                    ...
                    style_tags.append(f'<z class="dp{style_counter}">{css.strip()}</z>')
            ...
            for n in range(len(words)):
                if n == 0:
                    continue
                conn.execute(
                    """INSERT OR IGNORE INTO synonyms (synonym, word)
                VALUES (?, ?)""",
                    (words[n].lower().strip(), word),
                )

            if len(batch) == batch_size:
                batch_counter += 1
                print("Inserting data into dictionary", batch_size * batch_counter)
                conn.executemany(
        ...
        if batch:
            conn.executemany("INSERT INTO dictionary (word, defi) VALUES (?, ?)", batch)
    ...
    print(f"The size of the compressed dictionary.zip file is ~ {zip_size_mb:.2f} MB")

    with open(raw_extracted_styles, "w", encoding="utf-8") as css_file:
        css_content = "\n\n".join(style_tags).replace("\\n", " ")
        css_content = css_content.replace("<!DOCTYPE html>", "").replace(
            '<meta charset="utf-8">', " "
        )
